...
Agent PHẢI xuất kết quả theo đúng mẫu KBKT sau:
...
- Các bước thực hiện:
  - Đánh số từng bước.
  - Ghi rõ dữ liệu nhập.
  - Không sử dụng mô tả chung chung.

- Kết quả mong muốn:
  - Đánh số tương ứng với từng bước.
  - Mô tả rõ hành vi mong đợi của hệ thống.

- Kết quả hiện tại:
  - Mặc định: Chưa thực hiện

- Mã lỗi:
  - Để trống khi sinh testcase.
...
2. Sinh test case theo chuẩn KBKT:
...
- Mục đích kiểm thử:
  Mô tả ngắn gọn mục tiêu kiểm tra.

- Các bước thực hiện:
  Liệt kê đầy đủ từng bước thao tác.
...
### Bước 6: KBKT Template Mapping

Mục đích:

Chuẩn hóa toàn bộ testcase sang định dạng KBKT.

Agent phải:

1. Mapping toàn bộ testcase vào bảng:

| Mã trường hợp kiểm thử | Mục đích kiểm thử | Các bước thực hiện | Kết quả mong muốn | Kết quả hiện tại | Mã lỗi | Ghi chú |

2. Quy tắc:

- Không được bỏ sót testcase nào đã sinh ở Bước 5.
...
3. Nếu số lượng testcase lớn:

- Chia theo Module.
- Chia thành Part 1, Part 2...
- Không được rút gọn testcase.

Output:

Bảng KBKT hoàn chỉnh sẵn sàng import Excel.
...
---
## KBKT Generation Rules

BẮT BUỘC áp dụng cho mọi testcase được sinh ra.

Mỗi chức năng phải bao phủ đầy đủ:
...
Khi phát hiện tài liệu chưa rõ:

Agent phải sinh thêm:
...
Không được tự suy diễn nghiệp vụ.
